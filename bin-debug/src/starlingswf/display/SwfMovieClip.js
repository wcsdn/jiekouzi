...
(function (starlingswf) {
    var SwfMovieClip = (function (_super) {
        ...
        SwfMovieClip.prototype.getChildByName2 = function (name) {
        ...
        SwfMovieClip.prototype.addFrameScript = function (arr) {
            this.frameArr = arr;
        };
        SwfMovieClip.prototype.exceFunction = function () {
            if (this.frameArr == null || this.frameArr.length <= 0)
                return;
            var valfun;
            for (var i = 0; i < this.frameArr.length; i += 2) {
                if (this.frameArr[i] == (this._currentFrame + 1)) {
                    valfun = this.frameArr[i + 1];
                    if (valfun) {
                        valfun.call();
                        return;
                    }
        ...
        SwfMovieClip.prototype.gotoAndStop = function (frame, stopChild) {
            ...
            if (typeof (frame) == "number") {
                frame = (parseInt(frame.toString()) - 1);
            }
            this.goTo(frame);
            this.stop(stopChild);
        };
